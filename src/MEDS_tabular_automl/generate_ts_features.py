...
def feature_name_to_code(feature_name: str) -> str:
    """Converts a feature name to a code name by removing the aggregation parts.
    
    Robustly handles both single suffixes (e.g., /code) and double suffixes 
    (e.g., /value/max) generated by tabular AutoML pipelines.
    """
    # 1. Drop the final aggregation suffix (like /max, /count, or /present)
    base = "/".join(feature_name.split("/")[:-1])
    
    # 2. If it still ends in /value or /code, drop that too!
    if base.endswith("/value") or base.endswith("/code"):
